...
{
  ***********************************************************
  * Gerd's AVR-Assembler Version 3.3 as of June 07, 2012    *
  * Free Pascal, source code, (C)2012 by DG4FAC             *
  * New in Version 3.3: See ReadMe.Txt for version info     *
  ***********************************************************
}
Program gavrasm;
...
Const
  ...
  asHeader:Array[1..nHeader] Of String=(
    ...
    '+------------------------------------------------------------+');
  lHeader:String='gavrasm Gerd''s AVR assembler version 3.3 (C)2012 by DG4FAC';
  cTab=Char(9);
  iMaxInt:LongInt=$3FFFFFFF; { 1073741823 }

Var sfn:String; { filename of the source file }
    sListFile,sEepFile,sCodeFile,sXErrorFile:String; { Result files }
    fListFileOpen:Boolean; { List File is open }
    fQuiet,fSymbols,fList,fListOff,fAnsiOff,fErrLong,fWrapOn,fWrap,
    fTypesList,fOptionList,fListMac,fBeginner,fListDirectives,
    fInternalOff:Boolean; { Command line options }
    ...
    fPcX:Boolean; { Suppresses PC+X+1, adds PC+X }
...
If fTypesList Or fOptionList Or fListDirectives Then
  ...
  If fOptionList Then
    Begin
    Writeln;
    Writeln('Calls: gavrasm [-ABEJLMQSWX] source[.asm]');
    Writeln('       gavrasm [-?DHT]');
    Writeln;
    Writeln('List of options:');
    Writeln('  -L Listfile off            -M List macros');
    ...
    Writeln('  -X Internal def.inc off    -J Switch jump PC+X+1 to PC+X');
...
Procedure Error(ne:Byte;s1,s2:String); { Process error messages }
Begin
...
Procedure Warning(f:Boolean;nws:Byte;s1,s2:String); { List warnings }
Begin
Inc(nw);
If Not fQuiet Then
  Begin
...
Procedure Message(s:String); { Display a message }
Begin
Writeln('  ==> Message: ',s);
If Not fListOff Then
  Begin
...
Function LongInt2HexN(n:Byte;i:LongInt):String;
Var h:LongInt;
  b:LongInt;
  s:String;
Begin
...
Repeat
  b:=i Div h;
  i:=i-b*h;
  If b>9 Then s:=s+Char(b+55) Else s:=s+Char(b+48);
  h:=h Div 16;
  Until h<=1;
If i>9 Then s:=s+Char(i+55) Else s:=s+Char(i+48);
LongInt2HexN:=s;
End;

Function Longint2Hex(i:LongInt):String;
Var j,k:LongInt;
  s:String;
  f,fs:Boolean;
Begin
...
Repeat
  ...
  If f Then
    Begin
    If k>9 Then s:=s+Char(k+55) Else s:=s+Char(k+48);
    End Else
    s:=s+' ';
  i:=i-k*j;
  j:=j Div 16;
...
Const sListTypes='TLMGEDRCV';
  sConst='CZNHITVS';
Var k:LongInt;
  p:pSList;
Begin
ConcatAllMacroLabels;
Writeln(fl);
If FindAnySymbol Then
  ...
  For k:=1 To Length(sListTypes) Do
    ...
    While p<>NIL Do With p^ Do
      Begin
      If (Not fDefInc) And (Not ((cType='C') And (Length(sName)=1) And (Pos(sName,sConst)>0))) Then
        Writeln(fl,'  ',cType,nDefined:6,nUsed:6,iValue:12,Longint2Hex(iValue),' ',sName);
      p:=FindSymbolNext;
      End
    End;
  End Else
  Writeln(fl,GetMsgM(10));
RemoveAllMacroLabels;
End;
...
If Not CheckAllSymbols(nUnused,nVarReDef) Then
  Begin
  If nUnUsed>0 Then
    Warning(False,1,IntToStr(nUnused),'');
  If nVarReDef>0 Then
    Warning(False,2,'','');
  End;
End;
...
Begin
Str(npLine:6,ListGetLine);
ListGetLine:=ListGetLine+': ';
End;
...
If c=' ' Then For k:=1 To nDevices Do With aDevices[k] Do
  ...
  If (k Mod 21)=1 Then
    ...
    Writeln('                    Bytes  Bytes Adrs.  Bytes (Flags)');
    End;
  Write(k:3,' ',sn);
  For l:=Length(sn)+1 To 14 Do Write(' ');
...
Function GetInnerBracket(s:String;Var pka,pke:LongInt):Boolean;
Var k,nke,mke:Byte;
  f,fl:Boolean;
Begin
f:=True;
...
Function GetBinaryValue(s:String;Var i:LongInt):Boolean;
Var k:Byte;
Begin
GetBinaryValue:=True;
i:=0;
...
Function GetHexValue(s:String;Var i:LongInt):Boolean;
Var k:Byte;
Begin
GetHexValue:=True;
i:=0;
...
Function GetDecimalValue(s:String;Var i:LongInt):Boolean;
Var p:Byte;
Begin
...
Function GetLiteralValue(s:String;Var i:LongInt):Boolean;
Begin
GetLiteralValue:=(Length(s)=3) And (s[3]=s[1]);
If GetLiteralValue Then i:=Byte(s[2]);
End;

Function GetPCvalue(s:String;Var i:LongInt):Boolean;
Begin
...
Function GetSingleValue(s:String;Var fValid:Boolean;Var i:LongInt):Boolean;
Var p:Byte;
  fs:Boolean;
Begin
If s='-0' Then s:='0';
...
Case s[p] Of
  'A'..'Z','_':
    Begin
    fValid:=True;
    fValid:=GetPCvalue(Copy(s,p,255),i);
    If fValid Then GetSingleValue:=True Else
      Begin
      fValid:=True;
      fValid:=GetSymbolValue('CVLGMRP',Copy(s,p,255),fValid,i);
      If fValid Or (pass=1) Then GetSingleValue:=True Else
        Begin
        fValid:=False;
        Error(7,s,'');
        End;
  ...
  '0': If Length(s)>2 Then
    ...
    If s='0' Then
      ...
      GetSingleValue:=True;
      End Else
      Dbg('GetSingleValue: Don''t understand "'+s+'"!');
    End;
...
Function GetExpressionRight(s:String;Var p:LongInt;Var fValid:Boolean;Var i:LongInt):Boolean;
Var sh:String;
Begin
...
Function GetExpressionLeft(s:String;Var p:LongInt; Var fValid:Boolean;Var i:LongInt;fChkSigned:Boolean):Boolean;
Var sh:String;
Begin
...
Const
  as:Array[1..9] Of String = ('&&','||','!=','==','>=','<=','<>','>','<');
Begin
bType:=0;
Repeat Inc(bType) Until (bType>9) Or ((Pos(as[bType],s)>0) And ((bType<8) Or (s[Pos(as[bType],s)+1]<>as[bType])));
GetLogicOp:=bType<=9;
If GetLogicOp Then p:=Pos(as[bType],s);
End;

Function GetBitOp(s:String;Var p,bType:LongInt):Boolean;
Const
  as:Array[1..5] Of String = ('<<','>>','&','^','|');
Begin
bType:=0;
Repeat Inc(bType) Until (bType>5) Or (Pos(as[bType],s)>0);
GetBitOp:=bType<=5;
If GetBitOp Then p:=Pos(as[bType],s);
End;
...
Function GetValue(s:String;Var fValid:Boolean;Var i:LongInt):Boolean;
  ...
  If GetValue Then
    ...
    Case bType Of
      1:Begin
        ...
        if e1<=e2 Then
          i:=i1 SHL i2 Else {<<}
          Error(9,IntToStr(i1),IntToStr(i2));
        End;
      2:i:=i1 SHR i2; {>>}
      3:i:=i1 AND i2; {&}
      4:i:=i1 XOR i2; {^}
...
While GetValue And GetCalcOp(s,p,bType) Do
  ...
  pa:=p;
  pe:=p;
  GetValue:=GetExpressionLeft(s,pa,fValid,i1,True) And GetExpressionRight(s,pe,fValid,i2);
  If Getvalue Then
    Begin
    i:=0;
    Case bType Of
...
Function GetFunction(s:String;Var pka,i:LongInt):Boolean;
Const
  nas=11;
  as:Array[1..nas] Of String = ('LOW','HIGH','BYTE1','BYTE2','BYTE3','BYTE4',
...
If pka>3 Then
  ...
  If sh<>'' Then
    Begin
    k:=0;
    Repeat Inc(k) Until (k>nas) Or (sh=as[k]);
    If k<=nas Then
      Begin
      Case k Of
        1,3:i:=i MOD 256;
        2:If (i<0) And (i>-256) Then i:=255 Else i:=i DIV 256;
        4:If (i<0) And (i>-256) Then i:=255 Else i:=(i DIV 256) MOD 256;
        5:If (i<0) And (i>-65536) Then i:=255 Else i:=(i DIV 65536) MOD 256;
        6:If (i<0) And (i>-16777216) Then i:=255 Else i:=(i DIV 16777216) MOD 256;
        7:If (i<0) And (i>-65536) Then i:=65535 Else i:=i MOD 65536;
        8:i:=(i DIV 65536) MOD 65536;
        9:i:=(i DIV 65536) MOD 64;
        10:i:=2 SHL (i-1);
        11:i:=LongInt(TRUNC(LN(i)/LN(2.0)));
        End;
      If i<0 Then
        Begin
        i:=-i;
        If i<256 Then i:=256-i Else
        If i<65536 Then i:=65536-i Else
        If i<16777216 Then i:=16777216-i;
	End;
      pka:=p+1;
      End Else
      Begin
      GetFunction:=False;
      Error(13,Copy(s,p+1,pka-p),'');
      End;
    End;
  End;
End;
...
Function GetExpression(s:String;Var fValid:Boolean;Var i:LongInt):Boolean;
Var pka,pke:LongInt;
Begin
fValid:=True;
GetExpression:=True;
...
Function GetConstantLongInt(s:String;Var fValid:Boolean;Var i:LongInt):Boolean;
Begin
...
If SnipOff(s) Then
  ...
  If (Length(s)>1) And (s[1]='R') Then
    Begin
    Val(Copy(s,2,255),wr,e);
    If fAvr8L Then
      begin
      If (e<>0) Or (wr<16) Or (wr>31) Then
        Error(17,IntToStr(wr),'16..31') Else
        GetRegister:=True;
      end else
      begin
      If (e=0) And (wr<32) Then GetRegister:=True;
      end;
    End;
  If Not GetRegister Then
    Begin
    fValid:=True;
    If GetSymbolValue('R',s,fValid,i) Then
      Begin
      If (pass=2) And (Not fValid) Then Error(7,s,' ') Else
        Begin
        If (i>=0) And (i<32) Then
          ...
          If fAvr8L Then
            begin
            If (i>=16) Then GetRegister:=True Else
              Error(17,IntToStr(i),'16..31');
            end else
            GetRegister:=True;
          End Else
          Error(17,IntToStr(i),'0..31');
        End;
      End Else
      Begin
      If pass=1 Then
...
If GetDoubleRegister Then
  ...
  Case wr Of
    ...
    Error(17,IntToStr(wr),'R24/26/28/30!');
    End;
  End;
End;
...
If GetMovwReg Then
  Begin
  If ((wr<0) Or (wr>30) Or Odd(wr)) Then
    Begin
    GetMovwReg:=False;
    Error(17,IntToStr(wr),GetMsgM(39));
    End Else
    wr:=Word(wr Div 2);
  End;
End;
...
Function GetConstant(s:String;Var i:LongInt;imin,imax:LongInt):Boolean;
Var fValid:Boolean;
Begin
fValid:=True;
...
If GetConstant Then
  ...
  If (i<imin) Or (i>imax) Then
    Begin
    GetConstant:=False;
    Error(24,IntToStr(i),IntToStr(imin)+'...'+IntToStr(imax));
    End;
  End Else
  Error(25,s,'');
End;
...
If pass=1 Then wr:=0 Else
  ...
  If (s[1]='+') Or (s[1]='-') Then
    ...
    Begin
    GetOffset:=GetExpression(s,fValid,i);
    If fPcX Then i:=i-pc Else i:=i-(pc+1);
    End;
  If GetOffset And fValid Then
    Begin
    fError:=False;
    If (i<-imax) Or (i>(imax-1)) Then
      Begin
      fError:=True;
      If fPcX Then i:=i+pc Else i:=i+pc+1;
      If fWrapOn Then
        ...
        If i>(pc+1) Then
          Begin
          If fPcX Then
            i:=i-(pc+aDevices[nDev].nf Div 2) Else
            i:=i-(pc+1+aDevices[nDev].nf Div 2);
          End Else
          Begin
          If fPcX Then
            i:=aDevices[nDev].nf Div 2-pc+i Else
      ...
      If fError Then
        Begin
        GetOffset:=False;
        Error(23,IntToStr(i),'-'+IntToStr(imax)+'..+'+IntToStr(imax-1));
        End Else
      End;
    End;
  If GetOffset And fValid Then
...
If RemoveBlankTab(s1) Or RemoveBlankTab(s2) Then
        ...
        Case Length(s2) Of
          1:If Inst.sM='ELPM' Then
              ...
              If Not CheckInstValid(hasElpmZ) Then
                Error(35,'ELPM r,Z',aDevices[nDev].sn);
              End Else
              Begin
              w1:=$0004;
              If Not CheckInstValid(hasLpmZ) Then
                Error(35,'LPM r,Z',aDevices[nDev].sn);
              End;
          2:If s2[2]='+' Then
            Begin
  ...
  If Inst.sM='ELPM' Then
    ...
    If Not CheckInstValid(hasElpm) Then
      Error(35,'ELPM',aDevices[nDev].sn);
    End Else
    Begin
    w1:=$05C8;
    If Not CheckInstValid(hasLpm) Then
      Error(35,'LPM',aDevices[nDev].sn);
    End;
  End;
End;
...
If s='Z+' Then
  ...
  If CheckInstValid(hasSpmZ) Then GetSpm:=True Else
    Error(35,'SPM Z+',aDevices[nDev].sn);
  End Else
  Begin
  If s='' Then
    Begin
...
begin
GetZ:=UpperCase(s)='Z';
If Not GetZ Then Error(102,Inst.sM,s);
end;
...
With Inst Do With cl Do
  Begin
  If Not CheckInstValid(wt) Then
    Begin
    { Writeln('Instruction set = ',ToHex(aDevices[nDev].iSet)); }
    Error(35,sM,aDevices[nDev].sn);
    End Else
    Begin
          ...
          '9': {LDS}
            ...
            If fAvr8L Then
              ...
              If (wp2<$40) Or (wp2>$BF) Then
                Error(24,IntToStr(Round(wp2) And $FFFF),'64..191') Else
	        Begin
                If wp2>=$80 Then
          ...
          'A': {STS}
            ...
            If fAvr8L Then
              ...
              If (wp1<$40) Or (wp1>$BF) Then
                Error(24,IntToStr(Round(wp1) And $FFFF),'64..191') Else
	        Begin
                If wp1>=$80 Then
...
If SplitPseudoEquation(s1,s2) Then
  ...
  Case cType Of
    'R':
      ...
      If GetSymbolValue('CVG',s1,fValid,i) Then
        Begin
        fv:=False;
        Error(40,s1,'constant or variable');
        End Else
        Begin
        fv:=GetRegister(s2,w);
        i:=w;
  ...
  If fv Then
    Begin
    Case pass Of
      1:If Not AddSymbol(cType,s1,fValid,i) Then Error(55,s1,'');
      2:If Not SetSymbol(cType,s1,fValid,i) Then Error(56,s1,'');
      End;
    End Else
    Error(41,'','');
{$IFDEF debug}
...
For k:=1 To cl.nsp Do
  Begin
  fValid:=False;
  If (Not GetSymbolValue('G',UpperCase(cl.asp[k]),fValid,i)) And
    (Not ExportMacroSymbol(Uppercase(cl.asp[k]))) Then Error(91,'','');
  End;
End;
...
If GetExpression(cl.asp[1],fValid,i) And fValid Then
  ...
  Case cSegm Of
    'C':If i>=pc Then pc:=i Else
      Error(46,IntToStr(i),'code segment');
    'D':If i>=pd Then pd:=i Else
      Error(46,IntToStr(i),'data segment');
    'E':If i>=pe Then pe:=i Else
      Error(46,IntToStr(i),'eeprom segment!');
    End;
  End Else
  Begin
...
Procedure ProcessIf(fina:Boolean); { Call of routine in gavrif }
Var fValid:Boolean;
  i:LongInt;
Begin
fValid:=True;
If fIf And fIfDef Then IfNew(True,fina) Else
  Begin
  If GetExpression(cl.asp[1],fValid,i) Then 
    Begin
    If fValid Then IfNew(i<>1,fina) Else
      Error(61,'','');
    End Else
    Error(62,'','');
  End;
...
Procedure ProcessIfDevice(fina:Boolean); { Call of routine in gavrif }
Var p:PSList;
Begin
If fIf AND fIfDef Then IfNew(True,fina) Else
  Begin
  ...
  If GetDeviceFromName(cl.asp[1])>0 Then
    ...
    p:=FindSymbolFirst('T');
    If p=NIL Then
      Error(7,cl.asp[1],'') Else
      IfNew(UpperCase(cl.asp[1])<>p^.sName,fina);
    End Else
    Error(92,cl.asp[1],'');
  End;
End;
...
If fIf Then
  Begin
  If Not fIfDef Then fIfDef:=True Else
    Begin
    fValid:=True;
    If GetExpression(cl.asp[1],fValid,i) Then
      Begin
...
Procedure ProcessIfDef(fina:Boolean); { call of routine in gavrif }
...
If fIf AND fIfDef Then IfNew(True,fina) Else
  Begin
  fValid:=True;
  f:=Not (GetSymbolValue(sAllTypes,cl.asp[1],fValid,i) And fValid);
  IfNew(f,fina);
  End;
End;

Procedure ProcessIfNDef(fina:Boolean); { call of routine in gavrif }
Var f,fValid:Boolean;
  i:LongInt;
Begin
If fIf AND fIfDef Then IfNew(True,fina) Else
  Begin
  fValid:=True;
  f:=GetSymbolValue(sAllTypes,cl.asp[1],fValid,i) And fValid;
  IfNew(f,fina);
  End;
End;
...
If cl.nDirective>0 Then
  ...
  If fe Then
    ...
    Case cl.nDirective Of
      ...
      iDirIf:ProcessIf(False); { .IF }
      iDirIfDef:ProcessIfDef(False); { .IFDEF }
      iDirIfDevice:ProcessIfDevice(False); {.IFDEVICE }
      iDirIfNDef:ProcessIfNDef(False); { .IFNDEF }
      ...
      iDirSetGlobal:ProcessSetGlobal; { .SETGLOBAL }
      iDirUndef:Undef; { .UNDEF }
      End;
  If (Not fListOff) And (sListLine2<>'') Then
    Writeln(fl,sListLine2);
  End Else
  Error(67,'','');
End;
...
If (cl.sDirective='ENDMACRO') Or (cl.sDirective='ENDM') Then
  ...
  Begin
  If (sLabel<>'') And Not AddSymbol('M',sLabel,True,pcm^.ipcc) Then
      Error(55,sLabel,'');
  If sDirective='SET' Then
    ...
    If Pos('=',asp[1])>0 Then
      ...
      Error(65,'','');
    End;
  If sMacro<>'' Then { macro call within a macro }
    Begin
    If FindMacroLength(sMacro,i) Then
      Begin
      pcm^.ipcc:=pcm^.ipcc+i;
      pcm^.ipct:=pcm^.ipcc;
      End Else
...
With cl Do
  ...
  If (sLabel<>'') Then { define/set label }
    ...
    Case pass Of
      ...
      2:Begin
        fValid:=False;
        If GetSymbolValue('LMG',sLabel,fValid,i) Then
          Begin
...
Begin
If fMacroDef Or (fIf And fIfDef) Then { Inside macro def or if }
  Begin
  If Not fListOff Then
    Writeln(fl,ListGetLine,cl.sLine);
  If fMacroDef Then { line inside macro definition }
    ...
    Case nDirective Of
      iDirIf:ProcessIf(True);
      iDirIfDevice:ProcessIfDevice(True);
      iDirIfDef:ProcessIfDef(True);
      iDirIfNDef:ProcessIfNDef(True);
...
With aDevices[nDev] Do
  Begin
  If pc>(nf Div 2) Then Error(81,IntToStr(pc),IntToStr(nf Div 2));
  If pe>ne Then Error(82,IntToStr(pe),IntToStr(ne));
  If (pd-nss)>ns Then Warning(False,5,IntToStr(pd-nss),IntToStr(ns));
  End;
If Not IfClear Then
  Error(99,'','');
If nErr>0 Then
...
nDev:=0;
fCStyle:=False;
If GetOptions Then { Get comman line options }
  Begin
